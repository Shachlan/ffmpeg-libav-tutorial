...
static struct SwsContext *conversion_context_from_codec_to_rgb(AVCodecContext *codec)
{
    ...
    return sws_getContext(width, height, codec->pix_fmt,
...
{
    ConversionContext *context = (ConversionContext *)calloc(1, sizeof(ConversionContext));
...
{
    return sws_scale(context->conversion_context, (const uint8_t *const *)frame->data, frame->linesize, 0, frame->height, context->rgb_buffer, context->linesize);
}

int convert_to_frame(AVFrame *frame, ConversionContext *context)
{
    return sws_scale(context->conversion_context, (const uint8_t *const *)context->rgb_buffer, context->linesize, 0, frame->height, frame->data, frame->linesize);
}
