...
struct VideoFormatConverter {
  ...
  VideoFormatConverter(std::shared_ptr<AVCodecContext> context, EncodingConverionTag);

  /// Construct a new converter, from the given \c context to RGB pixel data.
  VideoFormatConverter(std::shared_ptr<AVCodecContext> context, DecodingConverionTag);
  ...
  void convert_from_frame(std::shared_ptr<AVFrame> frame) noexcept;
  ...
  void convert_to_frame(std::shared_ptr<AVFrame> frame) noexcept;
