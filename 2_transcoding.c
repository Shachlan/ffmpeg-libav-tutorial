...
static int decode_packet(TranscodeContext *decoder_context, TranscodeContext *encoder_context,
                         AVPacket *packet, AVFrame *inputFrame, AVFrame *outputFrame,
                         struct SwsContext *img_convert_ctx, struct SwsContext *img_invert_ctx, int stream_index)
{
  ...
  while (response >= 0)
  {
    ...
    if (response >= 0)
    {
      if (codec_context->codec_type == AVMEDIA_TYPE_VIDEO)
      {
        ...
        invertFrame(outputFrame->data, inputFrame->width, inputFrame->height);
        // sws_scale(img_invert_ctx, outputFrame->data, outputFrame->linesize, 0, inputFrame->height, inputFrame->data, inputFrame->linesize);
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    if (!outputFrame->data[plane])
    {
      printf("undefined data at %d\n", i);
    }
    if (!outputFrame->linesize[plane])
    {
