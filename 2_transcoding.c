...
static int decode_packet(TranscodeContext *decoder_context, TranscodeContext *encoder_context, AVPacket *packet, AVFrame *input_frame, AVFrame *output_frame, int stream_index);
...
int main(int argc, char *argv[])
{
  ...
  setupOpenGL(videoEncodingContext->width, videoEncodingContext->height);
...
static int decode_packet(TranscodeContext *decoder_context, TranscodeContext *encoder_context, AVPacket *packet, AVFrame *input_frame, AVFrame *output_frame, int stream_index)
{
  ...
  while (response >= 0)
  {
    response = avcodec_receive_frame(codec_context, input_frame);
    ...
    if (response >= 0)
    {
      if (codec_context->codec_type == AVMEDIA_TYPE_VIDEO)
      {
        ...
        invertFrame(input_frame, output_frame);
        encode_frame(decoder_context, encoder_context, encoder_context->format_context, encoder_context->codec_context[stream_index], output_frame, stream_index);
      }
